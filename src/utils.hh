...
#define LLC_MISS_THRESHOLD_CYCLES (167)
#define SHARED_READONLY_FILE_PATH                                              \
  ("/home/robin/repo/CS7292/streamline/shared_readonly_file.txt")
 ...
 */
static int fail_if_pthrattr_mismatch(int check_policy, int check_prio,
                                     int cpu) {
  int policy, s;
  struct sched_param param;
  ...
  s = pthread_getschedparam(pthread_self(), &policy, &param);
  if (s != 0) {
    printf("Error:pthread_getschedparam\n");
    exit(1);
  }
  if (policy != check_policy) {
    printf("Error:sched_policy mismatch\n");
    exit(1);
  }
  if (param.sched_priority != check_prio) {
    printf("Error:sched_priority mismatch\n");
    exit(1);
  }
  if (sched_getcpu() != cpu) {
    printf("Error:sched_priority mismatch\n");
