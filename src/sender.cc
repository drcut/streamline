...
/*
 * Function to send 0/1 via Flush+Reload channel to Receiver (for Initial
 * Handshake) For a predefined clock length,
...
uint64_t *SHARED_ARRAY;    // Shared array used for communication  [**TODO**:
                           // Assign addresses from shared_file.txt ]
...
int main(int argc, char **argv) {
 ...
 Start Tx-Rx-Delay-Cycles: %llu\n",
  ...
  printf("Other Data-Structures Sizes: rx_time_obs:%2f MB, tx_payload:%.2f MB, "
         "rx_payload:%.2f MB.\n",
         1.0 * NUM_BITS_DEBUG_DTSTR * sizeof(uint64_t) / 1024 / 1024,
  ...
  for (bit_id = 0; bit_id < TRANSMITTED_BITS; bit_id++) {
    ...
    if ((bit_id % (TX_SYNC_BITFREQ)) == (TX_SYNC_BITFREQ - 1)) {
      ...
      register uint64_t sync_time0, sync_delta_time0, sync_time1,
      ...
      while (!sync_complete) {
        int llc_hit_count = 0;
        // a. Flush addr where Rx will communicate it has reached barrier
        // (sync_rxready_addr)
        _mm_clflush((const void *)sync_rxready_addr);
        _mm_clflush((const void *)sync_rxready_addr1);
        _mm_clflush((const void *)sync_rxready_addr2);
        _mm_mfence();
