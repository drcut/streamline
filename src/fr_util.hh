/* Initial Handshake of Sender & Receiver Adapted from Flush=Reload Code at
 ...
 */
inline __attribute__((always_inline)) CYCLES rdtscp(void) {
  CYCLES cycles;
 ...
 */
inline CYCLES get_time() { return rdtscp(); }
 ...
 */
inline __attribute__((always_inline)) CYCLES cc_sync(int CHANNEL_SYNC_TIMEMASK,
                                                     int CHANNEL_SYNC_JITTER) {
 ...
 */
inline __attribute__((always_inline)) void clflush(ADDR_PTR addr) {
...
/*
 * Parses the arguments and flags of the program and initializes the struct
 * config with those parameters (or the default ones if no custom flags are
 * given).
 */
void init_config(struct config *config, uint64_t &NUM_BITS, int argc,
                 char **argv) {
  ...
  if (filename != NULL) {
    ...
    void *mapaddr =
        mmap(NULL, DEFAULT_FILE_SIZE, PROT_WRITE, MAP_SHARED, inFile, 0);
