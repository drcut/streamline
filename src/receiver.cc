/* Initial Handshake using Flush+Reload between Sender & Receiver Adapted from
   "https://github.com/yshalabi/covert-channel-tutorial". Subsequent Streamline
   Attack based on code written by Gururaj Saileshwar, Georgia Tech. Copyright
   (C) 2020, Gururaj Saileshwar
*/
 ...
 * Detects a bit by repeatedly measuring the access time of the load from
 * config->addr and counting the number of misses for the clock length of
 * config->interval.
 *
...
bool detect_bit(struct config *config, int interval) {
  ...
  // Sync with sender
  CYCLES start_t =
      cc_sync(config->CHANNEL_SYNC_TIMEMASK, config->CHANNEL_SYNC_JITTER);
  ...
  while ((get_time() - start_t) < interval) {
    ...
    CYCLES s = get_time();
    // while (get_time() < s + 1500 && get_time() < start_t + interval) {
    //   continue;
    // }
  }
  // printf("miss: %d hit: %d\n", misses, hits);
...
//-------- Access Pattern ----------
// Access every 3n cacheline alternating between page 0 and page 1, then
// continue to pages 2,3 .. 4,5 ... and so on.
#define PG_NUM(l)                                                              \
...
// -------- Network Parameters  -------------
// LFSR Based Channel Encoding to modulate payloads (to allow pathological
// payloads)
...
std::vector<uint64_t> rxsync_reached_timevec, rxsync_start_timevec,
    rxsync_complete_timevec;
std::vector<uint64_t> txsync_reached_timevec, txsync_complete_timevec;
std::vector<uint64_t> debug_rxsync_time, debug_timeout_duration,
    debug_timeout_bitid;
...
int main(int argc, char **argv) {
  ...
  TRANSMITTED_BITS =
      NUM_BITS * (DATABLK_BITLEN + PARITY_BITLEN) / DATABLK_BITLEN;
#else
...
// Printing Inputs
 ...
 Start Tx-Rx-Delay-Cycles: %llu\n",
         NUM_BITS, payload_type.c_str(), LLC_HIT_THRESHOLD_CYCLES_COMM,
         LLC_HIT_THRESHOLD_CYCLES_SYNC, RX_DELAY_CYCLES);
  ...
  tx_time_obs_timestamp =
      (uint64_t *)malloc(NUM_BITS_DEBUG_DTSTR * sizeof(uint64_t));
  rx_time_obs_timestamp =
      (uint64_t *)malloc(NUM_BITS_DEBUG_DTSTR * sizeof(uint64_t));
  // Bits to be transferred
  ...
  // Print Preliminaries.
  printf("Array Size: %llu bytes (%.2f GB). Starting index is: %llu (%lluth "
         "page)\n",
         SHARED_ARRAY_NUMENTRIES * sizeof(SHARED_ARRAY[0]),
         SHARED_ARRAY_NUMENTRIES * sizeof(SHARED_ARRAY[0]) /
             (1024 * 1024 * 1024.0),
         BITID_2_ARRINDEX(SHARED_SEED), SHARED_SEED);

  printf("Other Data-Structures Sizes: rx_time_obs:%2f MB, tx_payload:%.2f MB, "
         "rx_payload:%.2f MB.\n",
         1.0 * TRANSMITTED_BITS * sizeof(uint64_t) / 1024 / 1024,
         1.0 * TRANSMITTED_BITS * sizeof(bool) / 1024 / 1024,
         1.0 * TRANSMITTED_BITS * sizeof(bool) / 1024 / 1024);
  ...
  printf("Receiver Process - PID:%llu, TID:%lu, CPU:%d\n", getpid(),
         syscall(__NR_gettid), sched_getcpu());
  display_thread_sched_attr();
  fail_if_pthrattr_mismatch(SCHED_FIFO, sched_get_priority_max(SCHED_FIFO),
                            rx_cpuid);
  ...
  for (uint64_t i = 0; i < TRANSMITTED_BITS; i++) {
    ...
    if (i % (DATABLK_BITLEN + PARITY_BITLEN) == (DATABLK_BITLEN - 1)) {
      ...
      // encode data
      int enc_bytelen =
          fec_secded7264_encode(8, datablk_bytes, enc_datablk_bytes);
  ...
  for (uint64_t rx_id = 0; rx_id < TRANSMITTED_BITS; rx_id++) {
    ...
    if ((rx_id % (TX_SYNC_BITFREQ)) == (TX_SYNC_BITFREQ - TX_SYNC_LAG_DELTA)) {
      ...
      register uint64_t sync_time0, sync_delta_time0, sync_time0_2,
          sync_delta_time0_2;
      register uint64_t sync_time1, sync_delta_time1, sync_time2,
          sync_delta_time2;
      register uint64_t sync_time0_21, sync_delta_time0_21, sync_time0_22,
          sync_delta_time0_22;
      ...
      register uint64_t rxsync_reached_donetime, rxsync_start_donetime,
          rxsync_complete_donetime;
      ...
      while (!(sync_complete)) {
        ...
        sync_delta_time0 = __rdtscp(&sync_junk) -
                           sync_time0; /* READ TIMER & COMPUTE ELAPSED TIME */

        sync_time1 = __rdtscp(&sync_junk); /* READ TIMER */
        sync_temp1 += *sync_rxready_addr1;
        sync_delta_time1 = __rdtscp(&sync_junk) -
                           sync_time1; /* READ TIMER & COMPUTE ELAPSED TIME */

        sync_time2 = __rdtscp(&sync_junk); /* READ TIMER */
        sync_temp2 += *sync_rxready_addr2;
        sync_delta_time2 = __rdtscp(&sync_junk) -
                           sync_time2; /* READ TIMER & COMPUTE ELAPSED TIME */
  ...
  long bit_period_cycles =
      (rx_end_time - rx_start_time) / rx_loop_count * 1.0; // cycles
 ...
 FinalCorrectSamples=%.2f\% (%llu/%llu). Tx1_to_0_errors=%.2f\%, Tx0_to_1_errors=%.2f\% . Total-1s-Perc=%.2f\%\n",
         ...
         (1.0 * DATABLK_BITLEN / packet_sz) * 1000000.0 / bit_period_us,
         100.0 * correct_samples / total_samples, correct_samples,
         total_samples, 100.0 * one2zero_error / tx_samples,
         100.0 * zero2one_error / tx_samples, 100.0 * total_ones / tx_samples);

  printf(
      "Packet-ErrorType: NoError, \t 1-Bit Error,\t >=2-Bit Errors: \t %.2f%%  \t %.2f%% \
 \t %.2f%% (%llu,%llu,%llu)/%llu. Bit-Error-Perc:(1-Bit,2+): %.2f%% \t %.2f%% \n",
      100.0 * zero_bit_error_blks / tot_blks,
      100.0 * one_bit_error_blks / tot_blks,
      100.0 * twoplus_bit_error_blks / tot_blks, zero_bit_error_blks,
      one_bit_error_blks, twoplus_bit_error_blks, tot_blks,
      100.0 * one_bit_error_blks / total_samples,
      100 - 100.0 * correct_samples / total_samples -
          100.0 * one_bit_error_blks / total_samples);

  printf("Transmission Error Rates: TxCorrectRate=%.2f\% (%llu/%llu).\
 Tx1to0_errors=%.2f\%, Tx0to1_errors=%.2f\%\n",
         100.0 * tx_correct_samples / tx_samples, tx_correct_samples,
         tx_samples, 100.0 * one2zero_error / tx_samples,
         100.0 * zero2one_error / tx_samples);
  printf("-----------------------------\n\n");
  ...
  for (int i = 0; i < NUM_BITS / TX_SYNC_BITFREQ; i++) {
    ...
    uint64_t rxsync_time =
        rxsync_complete_timevec[i] - rxsync_reached_timevec[i];
 ...
 RX_SYNCREACH_TIME, RX_SYNCSTART_TIME, RX_SYNCCOMPLETE_TIME \n");
  ...
  for (uint64_t i = 0; i < NUM_BITS; i++) {
    ...
    if ((i % HEARTBEAT_FREQ) == (HEARTBEAT_FREQ - 1)) {
      ...
      if (epoch_id % (TX_SYNC_BITFREQ / HEARTBEAT_FREQ) == 0) {
        ...
        RX_SYNCREACH_TIME, RX_SYNCSTART_TIME, RX_SYNCCOMPLETE_TIME
        printf("%llu \t %.2f\% \t %.2f\% \t %.2f\% \t %lld \t %lld \t %lld \t "
               "%d \n",
               epoch_id, 100.0 * epoch_correct_samples / epoch_num_samples,
               100.0 * epoch_one2zero_error / epoch_num_samples,
               100.0 * epoch_zero2one_error / epoch_num_samples,
               rxsync_reached_timevec[sync_id],
               rxsync_start_timevec[sync_id] - rxsync_reached_timevec[sync_id],
               rxsync_complete_timevec[sync_id] -
                   rxsync_reached_timevec[sync_id],
               rxsync_epoch_miss[sync_id]);
      }
  ...
  for (int j = 0; j < debug_rxsync_time.size(); j++) {
    printf("Bit-ID:%llu, RX-Sync-Delay:%llu, Rx-Sync-Timeout:%llu\n",
           debug_timeout_bitid[j], debug_rxsync_time[j],
           debug_timeout_duration[j]);
  }
